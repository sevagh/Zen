...
public:
	    ...
	    , hpss(zg::hps::PRealtimeGPU(( float )p.fs, p.hop, p.beta, io))
	{
		...
		if (!ring_buffer_in) {
			std::cerr << "unable to create ring buffer in: out of memory" << std::endl;
			std::exit(1);
		}

		ring_buffer_out = soundio_ring_buffer_create(soundio, capacity);
		if (!ring_buffer_out) {
			std::cerr << "unable to create ring buffer out: out of memory" << std::endl;
			std::exit(1);
		}
			...
			if (input_count >= p.hop && output_free_count >= p.hop) {
				...
				if (p.do_hps) {
					...
					auto percussive_limits
					    = std::minmax_element(io.host_out, io.host_out + p.hop);
