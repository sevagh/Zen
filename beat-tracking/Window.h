...
constexpr Window<WindowSize> calculate_hamming_window()
{
	...
	for (size_t n = 0; n < WindowSize; ++n) {
		window.data[n] = 0.54F - (0.46F * gcem::cos(2.0F * PI * ((n_val) / N)));
		n_val += 1.0F;
	}
	return window;
}
...
constexpr Window<WindowSize> calculate_blackman_window()
{
	...
	for (size_t n = 0; n < WindowSize; ++n) {
		{
			window.data[n] = 0.42F - (0.5F * gcem::cos(2.0F * PI * (n_val / N)))
...
constexpr Window<WindowSize> calculate_tukey_window()
{
	...
	for (int n = 0; n <= width; ++n) {
		window.data[n]
		    = 0.5F
		      * (1.0F
		         + gcem::cos(PI * (-1.0F + 2.0F * (( float )n) / alpha / N)));
	}
...
template <std::size_t WindowSize>
constexpr Window<WindowSize> Hamming
    = detail::calculate_hamming_window<WindowSize>();

template <std::size_t WindowSize>
constexpr Window<WindowSize> Hanning
    = detail::calculate_hanning_window<WindowSize>();

template <std::size_t WindowSize>
constexpr Window<WindowSize> Blackman
    = detail::calculate_blackman_window<WindowSize>();

template <std::size_t WindowSize>
constexpr Window<WindowSize> Tukey
    = detail::calculate_tukey_window<WindowSize>();
