...
class BufferedHPSS {
	public:
		...
		void do_hpss() {
			for (;;) {
				int input_count = soundio_ring_buffer_fill_count(ring_buffer_in)/sizeof(float);
				std::cout << "HPSS BACKGROUND TICK! input: " << input_count << std::endl;

				if (input_count >= hop) {
					std::size_t rw_amount = hop * sizeof(float);

					    char *read_ptr = soundio_ring_buffer_read_ptr(ring_buffer_in);
					    memcpy(io.host_in, read_ptr, rw_amount);
					    soundio_ring_buffer_advance_read_ptr(ring_buffer_in, rw_amount);

					    //hpss.process_next_hop();

					    char *write_ptr = soundio_ring_buffer_write_ptr(ring_buffer_out);
					    memcpy(write_ptr, io.host_in, rw_amount);
					    soundio_ring_buffer_advance_write_ptr(ring_buffer_out, rw_amount);
						input_count = soundio_ring_buffer_fill_count(ring_buffer_in);
				}

				int output_count = soundio_ring_buffer_fill_count(ring_buffer_out);
...
static void write_callback(struct SoundIoOutStream *outstream, int frame_count_min, int frame_count_max) {
    ...
    soundio_ring_buffer_advance_read_ptr(hpss->ring_buffer_out, read_count * outstream->bytes_per_frame);
}
...
int main(int argc, char **argv) {
    ...
    if (instream->layout_error) {
        fprintf(stderr, "unable to open input stream with layout: %s", soundio_strerror(instream->layout_error));
        return 1;
    }
    ...
    if (outstream->layout_error) {
        fprintf(stderr, "unable to open output stream with layout: %s", soundio_strerror(outstream->layout_error));
        return 1;
    }

    std::cout << "instream bytes per frame: " << instream->bytes_per_frame << std::endl;
    std::cout << "outstream bytes per frame: " << outstream->bytes_per_frame << std::endl;
    std::cout << "instream channels: " << instream->layout.channel_count << std::endl;
    std::cout << "outstream channels: " << outstream->layout.channel_count << std::endl;

    //std::exit(0);

    int capacity = 2 * microphone_latency * instream->sample_rate * instream->bytes_per_frame;

    hpss = new BufferedHPSS(microphone_latency, capacity, hpss_hop, fs, soundio);
